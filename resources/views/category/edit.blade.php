...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{URL::to('/category/update')}}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            <input type="hidden" value="{{$contentData->cat_id}}" name="cat_id"/>
            <div class="card ">
              ...
              <div class="card-body ">
                    ...
                    <div class="form-group{{ $errors->has('parent_cat') ? ' has-danger' : '' }}">
                      ...
                      <option value="0">N/A</option>
                        @foreach($parentCategory as $pc)
                            <option @if($pc->cat_id == $contentData->parent_cat_id) selected @endif value="{{$pc->cat_id}}">{{$pc->name}}</option>
                        @endforeach
                      </select>
                ...
                <div class="row">
                    <div class="image-view col-sm-4 pull-right">
                            <img src="{{ asset('public/category_images').'/'.$contentData->image }}" />
                        </div>
                    </div>
                </div>
